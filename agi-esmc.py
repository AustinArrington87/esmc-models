...
for grower in growers:
    partner_grower_id = grower.get('partner_grower_id')
    grower_name = grower.get('partner_grower_name')
    grower_id = grower.get('grower_id')
    if not partner_grower_id:
        print("Skipping grower with missing ID")
        continue

    enrollments_url = f'https://sustainability-api.farmobile.com/api/v1/growers/{grower_id}/enrollments'

    response = requests.get(enrollments_url, headers=headers)
    if response.status_code == 404:
        print(f"Enrollments not found for partner grower ID {partner_grower_id}")
        continue
    ...
    for enrollment in enrollments:
        enrollment.update({
            ...
            "partner_grower_name": grower_name,
            "grower_id": grower_id,
